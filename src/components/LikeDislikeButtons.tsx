  ...
  const handleLike = () => {
    if (!isAuthenticated) {
      toast({
        ...
        description: "Войдите в систему, чтобы оценить новость",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="flex items-center space-x-2">
      <Button
        ...
        onClick={handleLike}
      >
        <ThumbsUp size={18} className={`mr-1 ${userLiked ? 'text-green-500' : ''}`} />
      ...
      >
        <ThumbsDown size={18} className={`mr-1 ${userDisliked ? 'text-red-500' : ''}`} />
